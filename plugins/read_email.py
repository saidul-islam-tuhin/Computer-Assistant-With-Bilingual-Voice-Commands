...
def main(text,lang):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            file_path = os.getcwd()+'/plugins/client_id.json'
    ...
    category_personal = service.users().messages().list(userId='me', includeSpamTrash=False, labelIds = ['CATEGORY_PERSONAL'], q='is:unread').execute()
    category_personal_messages_id = category_personal.get('messages', [])
    # msg = service.users().messages().get(userId='me', id='16a0ca9c03782170').execute()
    # https://mail.google.com/mail/u/0/#search/rfc822msgid:16a0ca9c03782170


    # important = service.users().messages().list(userId='me', includeSpamTrash=False, labelIds = ['IMPORTANT'], q='is:unread').execute()
    # important_messages_id = important.get('messages', [])
    # print("Total unread important message:", len(important_messages_id))

    # if there have not any unread message
    if not category_personal_messages_id:
        if lang.lower() == 'bangla':
            speak_text = 'অপঠিত কোন বার্তা নেই'
        else:
        ...
        if lang.lower() == 'bangla':
            speak_text = "মোট {} টি অপঠিত বার্তা রয়েছে".format(total_unread_msg)
            asking_text = "যদি অাপনি অপঠিত বার্তাগুলো দেখতে চান তবে হ্যা বলুন অন্যথায় না বলুন"
        else:
            speak_text = "Total unread personal message:{}".format(total_unread_msg)
