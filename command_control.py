...
from plugins import greetings, speak_time, speak_date, calculation, read_email, search_mail, open_folder, translate, current_weather, google_search
...
def input_parser(commands_list, selected_lang, command, profile_info):
    ...
    if selected_lang.lower() == 'bangla':
        ...
        for cus_cmd in commands_list['custom']:
            ...
            for cmd in commands_list['bangla']:
                if command in cmd.split(';'):
                    index = commands_list['bangla'].index(cmd)
                    break
